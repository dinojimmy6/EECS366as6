...
void main(void)
{
   ...
   //gouraud shading
	if(shadingMode < 1) {
		...
		//vec4 mls2intensity = vec4(1.0, 1.0, 1.0, 0.0);
		//vec4 mls2ambient = vec4(.2, .2, .2, 0.0);
		vec3 L = normalize(Lpos.xyz - V1);
		vec3 V1 = normalize(V1 * -1);
		vec3 H = normalize(L + V1);
		if(illumMode == 0) {
			vec4 Idiff = mm.diffuse * mls.intensity * max(dot(N1,L), 0.0);  
			Idiff = clamp(Idiff, 0.0, 1.0); 
			IPhong = mm.ambient * mls.ambient +  Idiff + mm.specular * mls.intensity * pow(dot(N1, H), mm.shininess);
		}
		else {
			//cook model
			...
			Icook = mm.ambient * mls.ambient + mls.intensity * vec4(F0, 0.0) * dot(N1, L) * mm.diffuse + mm.specular * vec4(temp, 0.0) * mls.intensity;		
		}
  }
